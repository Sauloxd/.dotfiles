...
** UI
#+begin_src elisp :noeval
  (setq doom-font (font-spec :size 16 :family "0xProto Nerd Font Mono"))
#+end_src
** Spacing
#+begin_src elisp :noeval
...
#+end_src
* Navigation
** gb / gB — jump back/forward across any cursor jump
Evil's default is =C-o= / =C-i=, which collide with terminal. Mirror them to =gb= / =gB=.
=better-jumper= is already on via Doom defaults and records LSP/search/imenu jumps.
#+begin_src elisp
(after! evil
  (map! :nv "gb" #'better-jumper-jump-backward
        :nv "gB" #'better-jumper-jump-forward))
#+end_src

** LSP peek — preview defs/refs without leaving point
=lsp-ui= is enabled by default with =:tools lsp=. Peek shows a popup you can
dismiss without modifying your jump list.
#+begin_src elisp
(after! lsp-ui
  (map! :map lsp-ui-mode-map
        :nv "gp" #'lsp-ui-peek-find-definitions
        :nv "gP" #'lsp-ui-peek-find-references))
#+end_src

** Python LSP server required for gd/gD
Doom's =+lsp= flag enables =lsp-mode= but does not install a server. Install one:
#+begin_src sh :tangle no
npm install -g pyright
# or: pip install "python-lsp-server[all]"
#+end_src
Verify with =M-x lsp-doctor= in a Python buffer.

* Projectile
** Use git superproject root instead of submodule root
When working in a repo with submodules, projectile defaults to the submodule as the project root.
This makes it use the parent repo root instead, giving a holistic view of all changes.
#+begin_src elisp
(after! projectile
  (defun +projectile-root-in-superproject (dir)
    "Detect git superproject and use it as project root."
    (when-let* ((default-directory dir)
                (super (string-trim
                        (shell-command-to-string
                         "git rev-parse --show-superproject-working-tree 2>/dev/null")))
                (_ (not (string-empty-p super))))
      (file-name-as-directory super)))
  (add-to-list 'projectile-project-root-functions '+projectile-root-in-superproject))
#+end_src
...
;; Don't let lsp recursively watch huge generated/scraped data dirs.
;; lsp ignores .gitignore, so it must be told explicitly or it tries to
;; register tens of thousands of FSEvents watchers and hangs Emacs.
(after! lsp-mode
...
(after! dtrt-indent
  ...
  (add-to-list 'dtrt-indent-hook-mapping-list '(typescript-ts-mode javascript typescript-ts-mode-indent-offset))
  (add-to-list 'dtrt-indent-hook-mapping-list '(tsx-ts-mode javascript typescript-ts-mode-indent-offset)))
#+end_src
...
** markdown
Toggle between =markdown-mode= (edit) and =markdown-view-mode= (read-only rendered view).
=markdown-view-mode= ships with =markdown-mode= and is loaded by Doom's =:lang markdown=.
Bound to =SPC k m=.
#+begin_src elisp
(defun sxd/markdown-toggle-view ()
  "Toggle between markdown-mode and markdown-view-mode in the current buffer."
  (interactive)
...
Scale header faces so =#=, =##=, ... visually stack like rendered HTML headings.
Applies in both =markdown-mode= and =markdown-view-mode=.
#+begin_src elisp
(custom-set-faces!
  '(markdown-header-face-1 :inherit outline-1 :weight bold :height 1.6)
  '(markdown-header-face-2 :inherit outline-2 :weight bold :height 1.4)
  '(markdown-header-face-3 :inherit outline-3 :weight bold :height 1.2)
  '(markdown-header-face-4 :inherit outline-4 :weight bold :height 1.1)
  '(markdown-header-face-5 :inherit outline-5 :weight bold :height 1.05)
  '(markdown-header-face-6 :inherit outline-6 :weight bold :height 1.0))
#+end_src

Pixel-accurate table column alignment via =valign=. Pipes stay visible but columns
line up perfectly. =markdown-view-mode= derives from =markdown-mode=, so this
hook fires in both. Tables >4000 chars are skipped by default (=valign-max-table-size=).
#+begin_src elisp
(use-package! valign
  :hook (markdown-mode . valign-mode))
#+end_src

*** Browser preview via JS renderer
Render the current buffer in the browser using a self-contained web shell
(=md-preview.html=, symlinked to =doom-user-dir=) that renders client-side with
=markdown-it= + =highlight.js= — nicer than the default =markdown-preview= HTML.

The markdown is base64-encoded and injected into the shell (no temp =.md=, no
=file://= fetch/CORS issues, and =</script>= in the content can't break out).
Bound over the default =SPC m p=.
#+begin_src elisp
(defun sxd/markdown-preview-web ()
  "Render the current markdown buffer in the browser via md-preview.html.
Injects the buffer (base64 UTF-8) into the web shell, writes a temp HTML
file, and opens it with `browse-url'."
  (interactive)
  (let* ((shell (expand-file-name "md-preview.html" doom-user-dir))
         (md (buffer-substring-no-properties (point-min) (point-max)))
         (b64 (base64-encode-string (encode-coding-string md 'utf-8) t))
         (title (let ((s (or (buffer-name) "markdown")))
                  (dolist (pair '(("&" . "&amp;") ("<" . "&lt;") (">" . "&gt;")) s)
                    (setq s (string-replace (car pair) (cdr pair) s)))))
         (template (with-temp-buffer
                     (insert-file-contents shell)
                     (buffer-string)))
         (html (string-replace "__MD_CONTENT__" b64
                               (string-replace "__MD_TITLE__" title template)))
         (out (make-temp-file "md-preview-" nil ".html")))
    (with-temp-file out (insert html))
    (browse-url (concat "file://" out))))

(map! :after markdown-mode
      :map (markdown-mode-map gfm-mode-map)
      ;; Override the built-in C-c C-c preview/export-preview keys.
      "C-c C-c p" #'sxd/markdown-preview-web
      "C-c C-c v" #'sxd/markdown-preview-web
      ;; And the Doom localleader entry.
      :localleader
      "p" #'sxd/markdown-preview-web)
#+end_src
